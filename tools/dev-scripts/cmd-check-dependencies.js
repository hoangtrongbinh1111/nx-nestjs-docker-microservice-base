...
const checkEnvFiles = () => {
  ...
  if (!hostEnvExists || !qaEnvExists || !devEnvExists) {
    if (!hostEnvExists) {
      console.error('[GSV][Error] Please put .env.host file inside your ./config/env/ folder.');
    }
    if (!qaEnvExists) {
      console.error('[GSV][Error] Please put .env.qa file inside your ./config/env/ folder.');
    }
    if (!devEnvExists) {
      console.error('[GSV][Error] Please put .env.dev file inside your ./config/env/ folder.');
    }
    console.error('[GSV][Info] You can copy the template .env.${HOST_ENV} file from ./config/env folder for each environment.');
    process.exit(1);
  }
}
...
const checkCertificates = () => {
  ...
  if (!certificates.crtExists || !certificates.keyExists) {
    console.log('[GSV][Error] Please make sure you install mkcert before generating your certificates.')
    if (!certificates.crtExists) {
      console.error('[GSV][Error] Please generate a localhost-crt.pem file inside your ./config/certificates/ folder by using mkcert.');
    }
    if (!certificates.keyExists) {
      console.error('[GSV][Error] Please generate a localhost-key.pem file inside your ./config/certificates/ folder by using mkcert.');
    }
    process.exit(1);
  }
}
...
const printNodeVersionError = (data) => {
  console.error(`[GSV][Error] Please use the correct node version -> ${data?.nvmNodeVersion}`);
  console.error(`[GSV][Error] If you have already run 'pnpm install' or 'pnpm setup' please delete all **/*/node_modules folders inside your project.`);
}

const printDockerError = () => {
  console.error('[GSV][Error] Please install docker to run the project correctly.');
}
...
const checkDependencies = () => {
  ...
  if (!partialCheckEnabled) {
    ...
    if (isOnlyNode) {
      console.log('[GSV] Checking only node and nvm version.');
      request = [checkNodeVersion(), checkNvmVersion()]
    }
    if (isOnlyDocker) {
      console.log('[GSV] Checking only docker dependency validity.');
      request = [checkDocker()];
    }
  }
